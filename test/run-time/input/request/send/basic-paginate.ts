...
tape(__filename, async (t) => {
    ...
    const server = await new Promise<http.Server>((resolve) => {
        ...
        app.get("/flower", (req, res) => {
            const query = FlowerApi.routes.paginate.query("req.query", req.query);
            if (query.page == undefined) {
                query.page = 17;
            }
            if (query.rowsPerPage == undefined) {
                query.rowsPerPage = 32;
            }
            res.json(FlowerApi.routes.paginate.response.mapMappable(
                "response",
                {
                    rows : [
                        {
                            flowerId : BigInt(query.page+2),
                            name : "Flower " + (query.page+2),
                            wateredAt : new Date("2010-01-01 00:00:" + String(query.page+2).padStart(2, "0")),
                        },
                    ],
                    ...
                    info : {
                        ...
                        rowsFound : query.page*query.rowsPerPage,
                        pagesFound : query.page+1,
                    }
                }
            ));
        });
