...
tape(__filename, async (t) => {
    ...
    const server = await new Promise<http.Server>((resolve) => {
        ...
        app.get("/flower/:flowerId(\\d+)", (req, res) => {
            t.deepEqual(req.headers["my-header"], undefined);
    ...
    const flowerApi = new FlowerApi({
        ...
        onTransformResponse : (sendResult) => {
            return {
                ...sendResult.responseBody,
                name : (sendResult.responseBody as any).name + " - Transformed",
                transformed : true,
            };
        },
    });
    ...
    t.deepEqual(result.responseBody.name, "Rose - Transformed");
    //Transformation happens before mapping.
    //Mapping removes the `transformed` field.
    t.deepEqual((result.responseBody as any).transformed, undefined);
